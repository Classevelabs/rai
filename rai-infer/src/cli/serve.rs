//! `rai serve` — local web chat UI over a `.raimodel`.
//!
...
//! driving the local model via DNS rebinding.
...
pub fn run(args: &ServeArgs) -> Result<()> {
    ...
    let template = ChatTemplate::from_str_arg(&args.options.chat_template, &tokenizer);
